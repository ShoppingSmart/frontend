    ...
    <div class="">
      ...
      <div class="mt-6 border-t border-b py-2">
        <div class="flex items-center justify-between">
          ...
          <p class="font-semibold text-gray-900">${{ subTotal() / 100 }}</p>
        </div>
        <div class="flex items-center justify-between">
          <p class="text-sm font-medium text-gray-900">Tax</p>
          <p class="font-semibold text-gray-900">${{ tax() / 100 }}</p>
        </div>
      </div>
      <div class="mt-6 flex items-center justify-between">
        <p class="text-sm font-medium text-gray-900">Total</p>
        <p class="text-2xl font-semibold text-gray-900">${{ total() / 100 }}</p>
      </div>
    </div>
    <button v-on:click="placeOrder" class="mt-4 mb-8 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white">Place Order</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    tax() {
      ...
      this.items.forEach((item) => {
        amount += item.quantity * item.price * (item.category.tax / 100);
      });
